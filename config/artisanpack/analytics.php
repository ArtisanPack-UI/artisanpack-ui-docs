...
return [
    ...
    */
    ...
    |
    */
    'dashboard_driver' => env('ANALYTICS_DASHBOARD_DRIVER', 'inertia'),
    ...
    |
    */
    'dashboard_route' => env('ANALYTICS_DASHBOARD_ROUTE', 'dashboard/analytics'),
    ...
    |
    */
    'dashboard_middleware' => ['web', 'auth', 'verified', 'can:view-analytics'],
    ...
    */
    'local' => [
        ...
        */
        ...
        |----------------------------------------------------------------------
        |
        | When enabled, page views and events are dispatched as queued jobs
        | onto the queue named below and processed by an external worker.
        | Disabled by default for this docs site: the volume is low, no
        | dedicated worker listens on the `analytics` queue (the app's
        | `composer run dev` only starts a worker on the default queue),
        | and a silently-buffered queue means beacons never reach the
        | database. Set ANALYTICS_QUEUE_PROCESSING=true in production once
        | a Horizon supervisor is watching the `analytics` queue.
        |
        */
        'queue_processing' => env('ANALYTICS_QUEUE_PROCESSING', false),
    ...
    'privacy' => [
        ...
        */
        'excluded_paths' => [
            ...
            '/telescope/*',
            '/horizon/*',
            // Auth-flow URLs carry secret tokens in the pathname. They must
            // never land in analytics_page_views.url or the tracker's URL
            // property, so mirror the sensitive-path list used by the
    ...
    */
    'dashboard' => [
        ...
        */
        ...
        |----------------------------------------------------------------------
        |
        | Master switch for the dashboard query cache. When false, every
        | Overview / Pages / Traffic / Audience / Events query hits the
        | database directly — useful for local debugging when you want
        | to see beacons the instant they land. The cache_duration
        | below is ignored when this is false.
        |
        */
        ...
        |----------------------------------------------------------------------
        |
        | How long to cache dashboard queries in seconds. Applies to every
        | Overview / Pages / Traffic / Audience / Events tab query — the
        | Realtime tab is served fresh out of the app's own controller
        | and never hits this cache.
        |
        | Vendor default is 300 (5 min), which is too long for a
        | low-traffic docs site: an admin clicks around and sees their
        | own hits missing until the cache TTL expires. 60s keeps DB
        | pressure trivial while feeling live. To bypass caching
        | entirely for local debugging, set ANALYTICS_CACHE_ENABLED=false
        | in .env — this TTL is ignored when the master switch is off.
        |
        */
